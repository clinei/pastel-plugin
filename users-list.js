window.users_list = (function closure_users_list() { "use strict";

var users_list = document.getElementById("users-list");
var active_list = document.getElementById("users-active");
var absent_list = document.getElementById("users-absent");
...
function startTransition() {
	...
	// get height
	var height = users_list.getBoundingClientRect().height.toString(10).concat("px");
...
function _append(user) {
	...
	var element = makeUserListItemElement(user);
...
function _changeList(user) {
	var element = findUserListItemElement(user);
	if (element !== null) {
		if (user.active === true) {
